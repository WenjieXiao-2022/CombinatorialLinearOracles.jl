...
BirkhoffBLMO(dim, lower_bounds, upper_bounds, int_vars; append_by_column = true) =
	BirkhoffBLMO(append_by_column, dim, lower_bounds, upper_bounds, int_vars, Int[], Int[], collect(1:dim), collect(1:dim), 1e-6, 1e-3)
...
Computes the extreme point given an direction d, the current lower and upper bounds on the integer variables, and the set of integer variables.
"""
function Boscia.compute_inface_extreme_point(
	blmo::BirkhoffBLMO,
	direction,
	x;
	kwargs...,
...
function Boscia.dicg_maximum_step(blmo::BirkhoffBLMO, direction, x; kwargs...)
	...
	direction =
		blmo.append_by_column ? reshape(direction, (n, n)) : transpose(reshape(direction, (n, n)))
	x = blmo.append_by_column ? reshape(x, (n, n)) : transpose(reshape(x, (n, n)))
